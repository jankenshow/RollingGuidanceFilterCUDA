...
__global__ void rgf_bilateral_kernel(const float* input, const float* guide,
  ...
  for (int dy = -radius; dy <= radius; ++dy) {
    for (int dx = -radius; dx <= radius; ++dx) {
      ...
      float range = expf(-((guide[nidx] - center) * (guide[nidx] - center)) /
...
void rolling_guidance_filter_cuda(const float* input, float* output, int width,
  ...
  // cudaMemcpy(d_ping, input, bytes, cudaMemcpyHostToDevice); // initial
  // guidance = input
  ...
  if (channels == 1) {
    rgf::gaussian_blur_kernel<<<blocks, threads>>>(d_input, d_ping, width,
                                                   height, sigma_s);
  } else {
    rgf::gaussian_blur_kernel_multi<<<blocks, threads>>>(
        d_input, d_ping, width, height, channels, sigma_s);
  }

  for (int i = 1; i < iterations; ++i) {
    if (channels == 1) {
      rgf_bilateral_kernel<<<blocks, threads>>>(d_input, d_ping, d_pong, width,
                                                height, sigma_s, sigma_r);
    } else {
      rgf_bilateral_kernel_multi<<<blocks, threads>>>(
